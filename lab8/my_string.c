...
{
	String *current_string = (String*)current_word_family;
	String *prev_String = (String*)word;
	String *computed_new_key = (String*)new_key;
	int i;
	char ch;
	if (my_string_get_size(current_string) != my_string_get_size(prev_String))
	{
		return FAILURE;
	}
	...
	//get prev string length
	int str1_length=strlen(prev_String->data);

	//ensure current string is lowercase
	for (i = 0; i < str1_length; i++)
	{
		current_string->data[i]=tolower(current_string->data[i]);
	}
	
	for (i = 0; i < str1_length; i++)
	{
		my_string_push_back(computed_new_key, prev_String->data[i]);
	}
	...
	for (i = 0; i < str1_length; i++)
	{
		...
		if (current_string->data[i] == ch)
		{
			computed_new_key->data[i] = ch;
	...
	printf("%s", computed_new_key->data);
	printf("\n");
